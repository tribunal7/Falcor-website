...
export default function QuoteForm({product}:{product?:string}){
  ...
  return <form className="quote-form" name="falcor-quote" method="POST" data-netlify="true" data-netlify-honeypot="bot-field" action="/thank-you/">
    ...
    <div className="form-grid">
      ...
      <label>Product of Interest<select required name="product" value={selectedProduct} onChange={e=>setSelectedProduct(e.target.value)}><option value="">Select a product</option>{products.map(p=><option key={p.slug} value={p.name}>{p.name}</option>)}</select></label>
      <label>Approx. Square Footage<input required name="square-footage" type="number" min="1"/></label>
      <label>Delivery ZIP / Project Location<input required name="zip" autoComplete="postal-code"/></label>
      <label>Project Timeline<input required name="timeline" placeholder="e.g. 4-8 weeks"/></label>
    </div>
